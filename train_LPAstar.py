...
def LPA_star(start, end, image_line, GPS, model, device, rhs, g, km):
    frontier = []
    heapq.heappush(frontier, (calculate_key(start, end, rhs, g, km), start))
    came_from = {}
    cost_so_far = {}
    came_from[start] = None
    cost_so_far[start] = (rhs[start], g[start])
    ...
    while len(frontier) > 0:
        ...
        if calculate_key(current, rhs, g, km) < cost_so_far[current]:
            heapq.heappush(frontier, (calculate_key(current, rhs, g, km), current))
        else:
            rhs[current] = float('inf')
            for next_node in neighbors(current, image_line):
                next_tensor = torch.tensor(next_node, dtype=torch.float32, device=device).unsqueeze(0)
                current_tensor = torch.tensor(current, dtype=torch.float32, device=device).unsqueeze(0)
                new_cost = g[current] + model_distance(GPS.to(device) + next_tensor, GPS.to(device) + current_tensor,
                                                       model)
                if next_node not in cost_so_far or new_cost < g[next_node]:
                    g[next_node] = new_cost
                    cost_so_far[next_node] = (rhs[next_node], g[next_node])
                    came_from[next_node] = current
                    heapq.heappush(frontier, (calculate_key(next_node, rhs, g, km), next_node))

    return came_from, rhs, g

def calculate_key(node, end, rhs, g, km):
    if node not in g:
        g[node] = float('inf')
    if node not in rhs:
        rhs[node] = float('inf')
    return min(g[node], rhs[node]) + km * heuristic(node, end)
...
def fit_one_epoch(net, criterion, epoch, epoch_size, Epoch, cuda):
    total_loss = 0
    save_path = ("/home/rvl122/paper/main/checkpoints/LPA")
    end = None 
    ...
    with tqdm(total=epoch_size, desc='Epoch {}/{}'.format(epoch + 1, Epoch), postfix=dict, mininterval=0.3) as pbar:
        for i, (images_front, images_seg, images_satellite, image_line, GPS, target) in enumerate(dataloader):
            ...
            rhs = {}
            g = {}
            km = 1.0  # km is the weighting factor for the heuristic function
            came_from, rhs, g = LPA_star(start, end, image_line.squeeze().cpu().numpy(), GPS[0].cpu().numpy(), model, device, rhs, g, km)
            path = [end]
            current = end
            while current != start:
            ...
            # Combine the LPA* loss with the existing loss
            ...
            loss = (loss_lon + (loss_lat*1.2)) + lpa_loss
...
if __name__ == '__main__':
    ...
    path_GPS = "/home/rvl122/paper/dataset/mydataset/ITRI_Zhongxing/ITRI_Zhongxing_01/gpx/shift/shift_test3-5_01_o.txt"
